...
namespace database{
    Database::Database(){
        ...
        _connection_string = Poco::format(
            "host=%s;user=%s;db=%s;password=%s",
            std::string(std::getenv("MYSQL_HOST")),
            std::string(std::getenv("MYSQL_DATABASE")),
            std::string(std::getenv("MYSQL_USER")),
